defmodule ConsultaPex.Router.V1 do
  use Plug.Router
  require Logger
  ...
  get "/dni/:numero" do
    case SunatApi.consultar_dni(numero) do
      ...
      {:error, reason} ->
        Logger.warning("GET /v1/dni/#{numero} failed: #{format_error(reason)}")
  ...
  get "/ruc/:numero" do
    case SunatApi.consultar_ruc(numero) do
      ...
      {:error, reason} ->
        Logger.warning("GET /v1/ruc/#{numero} failed: #{format_error(reason)}")
        send_json(conn, 400, %{success: false, error: format_error(reason)})
    end
  end
